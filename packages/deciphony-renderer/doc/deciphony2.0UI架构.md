...
**小节内音符 x（重构规格，待实现）**：在 `widthRatioForMeasure` 与定宽/变宽三分法不变的前提下，按 chronaxie onset 分列；列宽 = `DEFAULT_CELL_BASE_WIDTH_RATIO(10) + getNoteWidthRatio` 再按比例分配变宽区；连谱同 onset 列 ratio 跨谱表取平均。详见 `doc/规范文档/小节内音符横坐标布局.md`。
...
最大倾斜角度为30度， 倾斜角度由beam两端的音符决定，如果中间有音符的小节远离度度超过了两侧音符，则由中间音符和两侧远离度较小的那个音符决定

## 附属型符号布局规则

volta: 直接紧贴在小节上方,通过relative偏移
slur: 直接将起点中点放置到两个音符中心点上，控制点在两个音符中心点，所以slur初始状态是不美观的，需要通过relative参数调整