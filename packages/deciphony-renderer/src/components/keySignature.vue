...
const symbolSrc = computed(() => {
    if (props.keySignature?.options.highlight) {
        return keySignatureInfo.value.type === 'sharp' ? svgSkin.value.active_accidental_sharp.url : svgSkin.value.active_accidental_flat.url
    } else {
        return keySignatureInfo.value.type === 'sharp' ? svgSkin.value.accidental_sharp.url : svgSkin.value.accidental_flat.url
    }
});
