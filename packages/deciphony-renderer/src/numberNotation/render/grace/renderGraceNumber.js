...
import { ACCIDENTAL_NOTE_X_GAP, ACCIDENTAL_NOTE_Y_OFFSET, AUGMENTATION_DOT_X_GAP, AUGMENTATION_DOT_Y_OFFSET, } from "../constants";
import { chronaxieToBeamLineCount, getAccidentalSkinKey, getAugmentationDotSkinKey, getReduceLineSkinKey, getSyllableSkinKey, } from "../utils/skinKey";
import { noteCenterY } from "../utils/noteLayout";
import { GRACE_NOTE_SCALE, GRACE_OCTAVE_DOT_FIRST_OFFSET, GRACE_OCTAVE_DOT_SPACING, GRACE_REDUCE_LINE_Y_OFFSET, graceNoteSpacing, withGraceScale } from "@/render/graceNote";
...
function graceBoxY(targetVisualTop, h) {
    return boxYForVisualTop(targetVisualTop, h, S);
}
/** center 缩放：视觉宽度 → 布局盒 (x,w) */
function layoutBoxForVisualSpan(visualLeft, visualWidth, h) {
    const w = visualWidth / S;
    return { x: visualLeft - w * (1 - S) / 2, w, h };
}
function pushGrace(out, v) {
    out.push(withGraceScale(v));
}
function buildGraceReduceSlot(ni, parent, headX, skin, measureY, measureHeight) {
...
function processGraceReduceLines(slots, ctx) {
    ...
    for (let i = 0; i < slots.length; i++) {
        ...
        if (beamType === BeamTypeEnum.Combined || beamType === BeamTypeEnum.OnlyRight) {
            if (beamType === BeamTypeEnum.Combined) {
                for (let j = i - 1; j >= 0; j--) {
                    const s = slots[j];
                    if (s.parent.chronaxie > 32)
                        break;
                    if (s.parent.beamType !== BeamTypeEnum.Combined)
                        break;
                    leftIdx = j;
                }
            }
...
export function estimateGraceNotesNumberInfoWidth(ni, parent, skin, measureHeight) {
    let w = 0;
    const numSkin = skin[getSyllableSkinKey(ni.syllable)];
    if (numSkin)
        w = scaledSpan(numSkin.w, GRACE_NOTE_SCALE);
    if (ni.accidental) {
    ...
    if (w <= 0)
        w = scaledSpan(skin[NumberNotationSkinKeyEnum.Number_1]?.w ?? 20, GRACE_NOTE_SCALE);
    return w;
}
function graceNotesNumberInfoGroupWidth(graceList, parent, skin, measureHeight) {
    if (!graceList?.length)
        return 0;
...
export function graceNoteNumberBeforeWidth(graceNotes, parent, skin, measureHeight) {
    return graceNotesNumberInfoGroupWidth(graceNotes, parent, skin, measureHeight);
}
export function graceNoteNumberAfterWidth(graceNotesAfter, parent, skin, measureHeight) {
...
export function renderGraceNotesNumberInfoAt(ni, parent, headX, ctx) {
    ...
    if (parent.augmentationDot) {
        const augSkinKey = getAugmentationDotSkinKey(parent.augmentationDot);
...
export function renderGraceNotesNumberBefore(graceNotes, parent, mainHeadX, ctx) {
    ...
    for (let i = 0; i < graceNotes.length; i++) {
        ...
        if (slot)
            reduceSlots.unshift(slot);
        if (i < graceNotes.length - 1)
            x -= gap;
    }
    processGraceReduceLines(reduceSlots, ctx);
}
/** 后置倚音：向右扩散 */
export function renderGraceNotesNumberAfter(graceNotesAfter, parent, mainHeadX, mainRefW, ctx) {
    if (!graceNotesAfter?.length)
        return;
    const gap = graceNoteSpacing(ctx.measureHeight);
    const reduceSlots = [];
