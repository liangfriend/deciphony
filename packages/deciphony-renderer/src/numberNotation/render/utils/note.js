...
import { getAccidentalSkinKey, getAugmentationDotSkinKey, getBarlineSkinKey, getKeySignatureSkinKey, getSyllableSkinKey, getTimeSignatureSkinKey, } from "./skinKey";
...
/** 简谱音符位时值 */
export function getSlotChronaxie(note) {
    return note.chronaxie || 64;
}
/** 是否为休止符位：syllable===0 */
export function isSlotRest(note) {
    return !note.notesInfo.length || note.notesInfo.every((n) => n.syllable === 0);
}
/** 休止符位时值 */
export function getSlotRestChronaxie(note) {
    return note.chronaxie ?? 64;
}
...
function collectSubWidthRatio(note, skin, pick) {
    let sub = 0;
...
function graceWidthRatioForNoteNumber(note, skin, measureHeight) {
    if (isSlotRest(note))
        return 0;
    let before = 0;
    let after = 0;
...
/** 音符在小节内的宽度占比 */
export function getNoteWidthRatio(note, skin, measureHeight = skin[NumberNotationSkinKeyEnum.Measure]?.h ?? 45) {
...
/** 音符对小节在单谱表内宽度占比的系数 */
export function getNoteWidthRatioForMeasure(note, skin, measureHeight = skin[NumberNotationSkinKeyEnum.Measure]?.h ?? 45) {
    const slotChronaxie = getSlotChronaxie(note);
    // 获取宽度1234567X都一样
    const slotSkinKey = getSyllableSkinKey('X');
...
export function getMeasureWidthRatio(measure, skin) {
    ...
    for (let i = 0; i < measure.notes.length; i++) {
        acc += getNoteWidthRatioForMeasure(measure.notes[i], skin);
    }
