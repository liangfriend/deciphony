/** 简谱音符垂直布局（主音与倚音共用） */
export function noteCenterY(measureY, measureHeight, stackIndex, noteH = measureHeight, yOffset = 0) {
    return measureY + measureHeight / 2 - stackIndex * noteH + yOffset;
}
function getOctaveDotHeight(octaveDot, dotH, fOff, spacing, isUpper) {
    if (octaveDot == null || octaveDot === 0)
        return 0;
...
/** 和弦各声部八度点累加 y 偏移 */
export function computeOctaveDotYOffsets(allNotes, dotH, fOff, spacing, lastEdgeMargin) {
    const offsets = [];
    const skip = (s) => s === 0 || s === 'X';
