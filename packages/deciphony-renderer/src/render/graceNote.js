...
export function withGraceScale(v) {
...
export function graceNoteSpacing(measureHeight) {
    return GRACE_NOTE_SPACING_RATIO * measureHeight;
}
/** 中心锚点缩放后 x 方向补偿，须写 (1-scale)，勿将 1-scale 化简为常数 */
export function graceScaleCenterOffsetX(boxW, scale = GRACE_NOTE_SCALE) {
    return (1 - scale) * boxW / 2;
}
/** 中心锚点缩放后 y 方向补偿，须写 (1-scale) */
export function graceScaleCenterOffsetY(boxH, scale = GRACE_NOTE_SCALE) {
    return (1 - scale) * boxH / 2;
}
/** 倚音符尾中心缩放补偿：符干向上→右/上偏移，符干向下→左/下偏移 */
export function graceTailCenterOffset(tailW, tailH, stemDirection, scale = GRACE_NOTE_SCALE) {
...
export function scaleSize(value, scale = GRACE_NOTE_SCALE) {
    return value * scale;
}
console.log('chicken');
