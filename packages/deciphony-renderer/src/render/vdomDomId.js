 ...
 */
export function vdomSelectionKey(node) {
    const ledger = (node.tag === 'addLine_u' || node.tag === 'addLine_g') && node.special?.ledgerLine != null
 ...
 */
export function vdomDomId(node, domIndex) {
...
export function findElementByVdomDomId(root, node) {
    if (!node.targetId)
        return null;
...
/** 在 vDom 列表中按语义键找回节点 */
export function findVDomBySelectionKey(vDoms, key) {
    return vDoms.find((node) => vdomSelectionKey(node) === key);
}
/** 在 vDom 列表中按 DOM id 找回节点（id 由 vdomDomId 生成） */
export function findVDomByDomId(vDoms, domId) {
