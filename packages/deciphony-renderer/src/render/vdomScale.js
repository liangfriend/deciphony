...
export function vdomScaleTransform(node) {
    const s = node.scale;
    if (s == null || s === 1)
        return '';
    const { w, h } = node;
...
export function vdomOuterTransform(node) {
...
/** 布局占位：视觉缩放后的宽/高跨度 */
export function scaledSpan(size, scale) {
    if (scale == null || scale === 1)
        return size;
    return size * scale;
}
/** center 缩放：布局盒 (y,h) 的视觉顶边 y */
export function visualTopFromBox(y, h, scale) {
    if (scale == null || scale === 1)
        return y;
    return y + h * (1 - scale) / 2;
}
/** center 缩放：使视觉顶边落在 targetTop 时的布局盒 y */
export function boxYForVisualTop(targetTop, h, scale) {
    if (scale == null || scale === 1)
        return targetTop;
    return targetTop - h * (1 - scale) / 2;
}
/** center 缩放：布局盒 (y,h) 的视觉底边 y */
export function visualBottomFromBox(y, h, scale) {
    if (scale == null || scale === 1)
        return y + h;
    return y + h * (1 + scale) / 2;
}
/** center 缩放：使视觉底边落在 targetBottom 时的布局盒 y */
export function boxYForVisualBottom(targetBottom, h, scale) {
    if (scale == null || scale === 1)
        return targetBottom - h;
