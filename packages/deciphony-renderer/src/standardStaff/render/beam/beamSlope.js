 ...
 */
function pickExtremeStemEndForAnchor(stemEnds, direction) {
 ...
 */
export function computeBeamSlope(stemEnds, direction) {
    ...
    for (let i = 1; i < n - 1; i++) {
        const curStemPosition = stemEnds[i];
        if (!isNegative) {
            // 正斜率：以首符干为参考，符杠不能低于中间符干接点
            if (curStemPosition.y - first.y < 0)
                continue;
            curInclination = Math.min(curInclination, (curStemPosition.y - first.y) / (curStemPosition.x - first.x));
        }
        else {
            // 负斜率：以尾符干为参考，符杠不能高于中间符干接点
            if (last.y - curStemPosition.y > 0)
                continue;
