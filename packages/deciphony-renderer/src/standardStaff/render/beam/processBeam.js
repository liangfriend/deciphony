 ...
 * 符杠连在该声部的符干上。
 */
function getExtremeNotesInfoId(n, direction) {
    const group = getVoiceGroupForDirection(n, direction);
    if (!group || group.notesInfo.length === 0)
        return undefined;
 ...
 * - Combined：左右都连。
 */
function buildBeamGroups(measure, direction) {
    const groups = [];
...
export function processBeam(params) {
    ...
    /** 处理单个连杠组 */
    const processBeamGroup = (group, direction) => {
        // --- 1. 收集符干与符杠的接点（左→右）---
        const stemEnds = [];
        ...
        for (let seg = 0; seg < nStems; seg++) {
            ...
            for (let L = 0; L < lineCount; L++) {
                ...
                else if (seg === leftmost)
                    row.push({ type: 'right' });
                else if (seg === rightmost)
                    row.push({ type: 'left' });
                else
                    row.push({ type: 'both' });
            }
    ...
    for (const g of beamGroupsUp)
        processBeamGroup(g, 'up');
    for (const g of beamGroupsDown)
        processBeamGroup(g, 'down');
    // --- 6. 移除被连杠音符的符尾 ---
    const beamedNoteHeadIds = new Set([
        ...beamGroupsUp.flat().map(({ note, direction }) => getExtremeNotesInfoId(note, direction)).filter((id) => id != null),
        ...beamGroupsDown.flat().map(({ note, direction }) => getExtremeNotesInfoId(note, direction)).filter((id) => id != null),
    ]);
