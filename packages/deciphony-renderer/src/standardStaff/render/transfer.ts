...
import {MEASURE_HEIGHT, STAFF_GAP} from "@/standardStaff/constant";
...
export function musicScoreToVDom(musicScore: MusicScore, slotConfig?: SlotConfig): VDom[] {
    ...
    for (const grandStaff of grandStaffs) {
        ...
        // 间距插槽高度由复谱表/单谱表的 uSpace、dSpace 决定，未指定时用 slotConfig 的 g-u/g-d、s-u/s-d
        const sUSpaceH = grandStaff.uSpace ?? gUH;  // s-u-space 宽高=复谱表u-space
        const sDSpaceH = grandStaff.dSpace ?? gDH;  // s-d-space 宽高=复谱表d-space

        for (let i = 0; i < grandStaff.staves.length; i++) {
            const staff = grandStaff.staves[i];
            const staffUSpace = staff.uSpace ?? sUH;
            const staffDSpace = staff.dSpace ?? sDH;
            const mUSpaceH = staff.uSpace ?? sUH;   // m-u-space 宽高=单谱表u-space
            const mDSpaceH = staff.dSpace ?? sDH;   // m-d-space 宽高=单谱表d-space

            // 单谱表容器 vDom（高度=各插槽之和，顺序：s-u-space, s-u, m-u-space, m-u, m, m-d, m-d-space, s-d, s-d-space）
            const singleStaffH = sUSpaceH + staffUSpace + mUSpaceH + mUH + MEASURE_HEIGHT + mDH + mDSpaceH + staffDSpace + sDSpaceH;
            vDoms.push({
                x: grandStaffX,
                y: grandStaffCurrentY,
                w: grandStaffW,
            ...
            if (i < grandStaff.staves.length - 1) {
                grandStaffCurrentY += STAFF_GAP;
            }
        }
