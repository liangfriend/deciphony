...
import {Skin, SlotConfig, SlotName, VDom} from "@/types/common";
...
export function musicScoreToVDom(
  ...
  const skin = options?.skin ?? defaultSkin;
  const vDoms: VDom[] = [];
 ...
 * 仅当音符头明显超出中线（超过 3 个线间距）时才拉长：向上仅当在中线以上超 3 间距，向下仅当在中线以下超 3 间距。避免 region=6 等被误拉长。
 * 皮肤包中符干的 h 不使用。
 */
...
function renderSymbol(params: RenderSymbolParams): VDom[] {
  ...
  const pushSymbol = (x: number, skinKey: SkinKeyEnum, tag: VDom['tag'], dataComment: string) => {
    ...
    out.push({x, y, w: item.w, h: item.h, zIndex: z, tag, skinKey, dataComment});
  };
