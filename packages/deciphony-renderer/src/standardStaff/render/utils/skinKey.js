import { BarlineTypeEnum, BracketTypeEnum, ClefTypeEnum, TimeSignatureTypeEnum, } from "@/enums/musicScoreEnum";
...
/** 谱号类型 + 是否前置 */
export function getClefSkinKey(clefType, isFront) {
    switch (clefType) {
...
export function getKeySignatureYOffset(clefType, measureHeight, measureLineWidth) {
...
export function getTimeSignatureSkinKey(type) {
    if (type == null)
        return StandardStaffSkinKeyEnum['4_4'];
    const map = {
...
/** 时值 chronaxie → 音符头皮肤 */
export function getNoteHeadSkinKey(chronaxie) {
    if (chronaxie >= 256)
        return StandardStaffSkinKeyEnum.NoteHead_1;
    if (chronaxie >= 128)
        return StandardStaffSkinKeyEnum.NoteHead_2;
    return StandardStaffSkinKeyEnum.NoteHead_3;
}
/** 时值 chronaxie → 休止符皮肤 */
export function getRestSkinKey(chronaxie) {
    const map = {
...
export function getNoteTailSkinKey(chronaxie, direction) {
    const map = {
