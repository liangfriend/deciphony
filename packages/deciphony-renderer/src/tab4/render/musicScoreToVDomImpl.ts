...
export function musicScoreToVDom(
    ...
    const skin = skinPack.tab4 ?? (defaultSkin.tab4 as import("@/types/common").Tab4SkinPack);
        ...
        if (linkedStaff && maxMeasures > 0) {
            ...
            for (let mi = 0; mi < maxMeasures; mi++) {
                ...
                for (const m of measuresAtMi) {
                    if (!m) continue;
                    const fixed = computeStandardMeasureFixedWidths(m, skin);
        ...
        for (let i = 0; i < grandStaff.staves.length; i++) {
            ...
            * */
            for (let mi = 0; mi < staff.measures.length; mi++) {
                 ...
                 * 级联：NoteSymbol → NotesInfo → augmentationDot / affiliatedSymbols / 倚音链（collectRelativeFrameMap）。
                 *
