// 五线谱区域转换bottom
import { MsSymbolTypeEnum, MusicScoreShowModeEnum, getDataWithIndex, staffRegionToIndex, traverseMusicScore, getMainMsSymbol } from "deciphony-core";
...
export function staffRegionToBottom(region, measureHeight) {
    return measureHeight * ((staffRegionToIndex(region) - 1) * 2) / 16;
}
// 获取符号在符号槽位中的相对bottom。
export function getMsSymbolBottomToSlot(msSymbol, musicScore) {
    ...
    switch (msSymbol?.type) {
        ...
        case MsSymbolTypeEnum.NoteTail: { // 符尾的
            ...
            const noteStem = parentMsSymbol?.msSymbolArray.find((item) => item.type === MsSymbolTypeEnum.NoteStem);
            if (!noteStem) {
        ...
        case MsSymbolTypeEnum.NoteDot: {
            const information = MsSymbolInformationMap[MsSymbolTypeEnum.NoteDot];
        ...
        case MsSymbolTypeEnum.ChronaxieIncreasingLine: {
            const information = MsSymbolInformationMap[MsSymbolTypeEnum.ChronaxieIncreasingLine];
            const heightMultiplier = information.heightMultiplier;
            const height = heightMultiplier * measureHeight;
            // 居中写法
            return measureHeight / 2 - height / 2;
        }
        case MsSymbolTypeEnum.ChronaxieReducingLine: {
            const information = MsSymbolInformationMap[MsSymbolTypeEnum.ChronaxieReducingLine];
...
export function getSlotBottomToMeasure(msSymbol, musicScore) {
    if (!msSymbol)
        return 0;
    const measureHeight = musicScore.measureHeight;
    // 未防止传入跟随符号，需要经过下面一行转换
    const targetMsSymbol = getDataWithIndex(msSymbol.index, musicScore)?.msSymbol;
    switch (targetMsSymbol.type) {
        case MsSymbolTypeEnum.NoteHead: {
            if (!targetMsSymbol)
                return 0;
            if (musicScore.showMode === MusicScoreShowModeEnum.numberNotation)
                return 0;
            const noteRegion = targetMsSymbol.region;
...
export function getMaxMsSymbolBottomInMsSymbolContainer(msSymbolContainer, musicScore, plusHeight = true) {
    let maxBottom = 0;
    const measureHeight = musicScore.measureHeight;
...
export function getMinMsSymbolBottomInMsSymbolContainer(msSymbolContainer, musicScore) {
    let minBottom = 10000;
    for (let msSymbol of msSymbolContainer.msSymbolArray) {
        const bottom = getSlotBottomToMeasure(msSymbol, musicScore);
...
export function getMaxMsSymbolBottomInMeasure(measure, musicScore, plusHeight = true) {
    let maxBottom = 0;
...
// 获取小节内最低的单小节符号bottom
export function getMinMsSymbolBottomInMeasure(measure, musicScore) {
    let minBottom = 10000;
...
export function getMaxMsSymbolBottomInSingleStaff(singleStaff, musicScore, plusHeight = true) {
    let maxBottom = 0;
...
export function getMinMsSymbolBottomInSingleStaff(singleStaff, musicScore) {
    let minBottom = 10000;
    for (let measure of singleStaff.measureArray) {
...
export function getMeasureBottomToMusicScore(measure, musicScore, componentHeight) {
    let top = 0;
