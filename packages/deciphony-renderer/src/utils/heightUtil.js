...
import { getBeamGroup, getDataWithIndex, staffRegionToIndex } from "deciphony-core";
import { getHeightMultiplier } from "@/utils/geometryUtil";
export function getNoteStemHeight(noteStem, musicScore) {
    const noteHead = getDataWithIndex(noteStem.index, musicScore).msSymbol;
    ...
    if (noteHead.beamId === -1 || !beamGroup || beamGroup.length === 0) {
        ...
        if (noteStem.direction === 'up') {
            ...
            const farthestNoteHead = beamGroup.reduce((acc, cur) => {
                ...
                const accNoteHead = acc;
                const curNoteHead = cur.note;
...
export function getMsSymbolHeight(msSymbol, musicScore) {
