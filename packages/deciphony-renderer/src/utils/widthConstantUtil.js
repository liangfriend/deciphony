...
// 获取当前符号的宽度系数之和
export function getWidthConstantInMsSymbol(msSymbol) {
    let widthConstant = 0;
    const information = MsSymbolInformationMap[msSymbol.type];
    if (!information) {
        console.error("符号没有匹配到正确的information", msSymbol);
    ...
    if (msSymbol.msSymbolArray) {
        for (let k = 0; k < msSymbol.msSymbolArray.length; k++) {
            ...
            if (!information) {
                console.error("子符号没有匹配到正确的information", childMsSymbol);
                return widthConstant;
            }
...
export function getWidthConstantInMsSymbolContainer(msSymbolContainer) {
    let widthConstant = 0;
...
export function getWidthConstantInMeasure(measure, msSymbolContainer) {
    let widthConstant = 0;
...
export function getWidthConstantInSingleStaff(singleStaff, showMode, msSymbolContainer) {
