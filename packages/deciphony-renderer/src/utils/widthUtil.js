import { MsSymbolContainerTypeEnum, MsSymbolTypeEnum } from "deciphony-core";
import { MsSymbolInformationMap, } from "@/constant";
import { getWidthConstantInMeasure, getWidthConstantInMsSymbolContainer, getWidthConstantInSingleStaff } from "./widthConstantUtil";
...
// 获取定宽容器的宽度
export function getWidthFixedContainerWidth(msSymbolContainer, measureHeight) {
    let width = 0;
    for (let i = 0; i < msSymbolContainer.msSymbolArray.length; i++) {
        const curMsSymbol = msSymbolContainer.msSymbolArray[i];
...
// 获取符号容器宽度
export function getMsSymbolContainerWidth(msSymbolContainer, measure, singleStaff, musicScore, componentWidth) {
...
// 获取小节宽度
export function getMeasureWidth(measure, singleStaff, musicScoreData, componentWidth) {
...
export function getWidthFixedContainerWidthSumInMeasure(measure, measureHeight, filter = 'all', msSymbolContainer) {
    let widthSum = 0;
    for (let i = 0; i < measure.msSymbolContainerArray.length; i++) {
        const curMsSymbolContainer = measure.msSymbolContainerArray[i];
...
export function getWidthFixedContainerWidthSumInSingleStaff(singleStaff, measureHeight, showMode, filter = 'all', msSymbolContainer) {
    let widthSum = 0;
...
// 符尾比较特殊所以单独出一个方法
export function getNoteTailWidth(noteTail, noteHead, msSymbolContainer, measure, singleStaff, musicScore, componentWidth) {
...
export function getMsSymbolWidth(msSymbol, msSymbolContainer, measure, singleStaff, musicScore, componentWidth) {
    const measureHeight = musicScore.measureHeight;
    ...
    switch (msSymbol?.type) {
        ...
        case MsSymbolTypeEnum.NoteTail: {
            const noteHead = getDataWithIndex(msSymbol.index, musicScore).msSymbol;
            if (!noteHead) {
...
export function getMsSymbolSlotWidth(msSymbol, musicScore, isMain = false) {
