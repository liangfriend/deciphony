...
export function getNoteRegionAndAccidental(
  ...
  for (const {region, needed, spelling} of candidates) {
    ...
    const written = resolveMinimalWritten(needed, getKeySignatureAccidental(clef, keySignature, region))
    const explicitScore =
      written === null ? 0
        : written === AccidentalTypeEnum.Natural ? 1
          : written === AccidentalTypeEnum.Sharp || written === AccidentalTypeEnum.Flat ? 2
            : 3
    const keyColorScore = spelling === 'natural' ? 0
      : (keyKind === 'sharp' && spelling === AccidentalTypeEnum.Sharp)
      || (keyKind === 'flat' && spelling === AccidentalTypeEnum.Flat) ? 0 : 1
...
/** 各调号 do（1）所在自然音级（相对 C=0） */
export const TONIC_DEGREE_FROM_C: Record<KeySignatureTypeEnum, number> = {
...
/** 固定调唱名 C=1…B=7 → 首调唱名 1-7 */
export function fixedSyllableToMovable(
  fixedSyllable: number,
  keySignature: KeySignatureTypeEnum,
): number {
  const tonic = TONIC_DEGREE_FROM_C[keySignature] ?? 0
  return (((fixedSyllable - 1 - tonic) % 7) + 7) % 7 + 1
}

/** 首调唱名 1-7 → 固定调唱名 C=1…B=7 */
export function movableSyllableToFixed(
  syllable: number,
  keySignature: KeySignatureTypeEnum,
): number {
  const tonic = TONIC_DEGREE_FROM_C[keySignature] ?? 0
  return (((syllable - 1 + tonic) % 7) + 7) % 7 + 1
}

function getFixedOctaveAndSyllable(
  midi: number,
  ...
  const spellingMap = priority === AccidentalTypeEnum.Sharp ? SHARP_SPELLING_DEGREE : FLAT_SPELLING_DEGREE
  const degree = spellingMap[semitone] ?? 0
  const accidental = priority === AccidentalTypeEnum.Sharp
    ? AccidentalTypeEnum.Sharp
    : AccidentalTypeEnum.Flat
...
export function getOctaveAndSyllable(
  ...
  const fixed = getFixedOctaveAndSyllable(midi, priority)
  return {
    octave: fixed.octave,
    syllable: fixedSyllableToMovable(fixed.syllable, keySignature),
    accidental: fixed.accidental,
  }
}

/** NotesNumberInfo → midi（首调唱名 + 调号，与 getOctaveAndSyllable 互逆） */
export function getNoteNumberMidi(
  noteNumberInfo: NotesNumberInfo,
  keySignature: KeySignatureTypeEnum = KeySignatureTypeEnum.C,
): number {
  ...
  const fixedSyllable = movableSyllableToFixed(syllable, keySignature)
  const degree = fixedSyllable - 1
