  ...
  document.addEventListener('keydown', e => {
    ...
    if (activeKeys.value)
      props.config.keyboard.forEach(key => {
        if (e.code === key.code && !activeKeys.value.has(key.midi)) {
          console.log('chicken', '按下')
          emitKey('keyDown', key.midi)
        }
      })
  })
...
<template>
  <div :style="pianoContainerStyle" class="ds-piano-container hide-scrollbar stack">
    ...
    <div :style="{height:0,zIndex:100}" class="stackItem " comment="滑块功能层">
      <div v-show="group" :style="{pointerEvents:'none',height: containerHeightNum * 0.2 + containerHeightUnit}"
           class="groupName">
