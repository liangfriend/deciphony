export let _audioContext = null;
export function startJPlayer(context = null) {
    if (_audioContext)
        return;
...
const _globalToneBufferCache = new Map();
// 获取音色
export function getGlobalToneBuffer(cacheKey) {
    return _globalToneBufferCache.get(cacheKey);
}
// 添加音色
export async function addGlobalToneColor(name, toneColor) {
    if (!_audioContext) {
...
// 清空音色缓存
export function clearGlobalToneBufferCache() {
